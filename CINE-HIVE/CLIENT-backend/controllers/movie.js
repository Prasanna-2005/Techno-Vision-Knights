...
getAllMoviesForGridview: async (req, res) => {
  try {
    ...
    const moviesWithImages = movies.map((movie) => {
      let posterImageUrl  = cloudinary.url(`grid_images/${movie.id}`)
      return {
        ...movie,
        posterImageUrl,
      };
  ...
  NAgetAllMoviesForGridview: async (req, res) => {
    try {
      ...
      // Map the movies with the same image URL
      const moviesWithImages = movies.map((movie) => {
        let posterImageUrl  = cloudinary.url(`grid_images/${movie.id}`)
        return {
          ...movie,
...
getMovieById: async (req, res) => {
  try {
    ...
    movie.posterimageUrl = cloudinary.url(`grid_images/${movie.id}`)
    movie.themeimageUrl  = cloudinary.url(`theme_images/${movie.id}`)
